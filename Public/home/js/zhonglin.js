$(document).ready(function() {
    ...
    $(".dianji-qh").dblclick(function(event) {
        $(".select-city").hide();
    });
...
//    $(function(){
//        function check(obj) {
//            $('.bank_end').attr('checked', false);//清空所有复选框
...
//轮播图 END
    ...
    //手动改变购买数量
    $("input[class='buygoodsnum']").change(function(){
        var vj=$(this).val();
        var dj=$(this).parent().next("p").children('span').text();
        if (vj>0) {
            $(this).parent().siblings(".mid-je").children('span').text(dj*vj);
        }else{
            $(this).val(1);
        }
    })
    ...
    //购物车全选
    $(".qx").click(function () {
        $(this).attr('checked')?$("input[data='hobby']").attr('checked',true):$("input[data='hobby']").attr('checked',false);
    })
    //商家全选
    $(".sjqx").click(function () {
        $(this).attr('checked')?$(this).parent().nextAll("div").children('input').attr('checked',true):$(this).parent().nextAll("div").children('input').attr('checked',false);
    })
    //单个商品删除
    $('.delete').click(function(){
        confirm("真的要删除吗？")?$(this).parent().parent().remove():"";
        for(var i in $('.info-top')){
            $('.info-top').eq(i).siblings().hasClass('info-mid')?"":$('.info-top').eq(i).remove();
        }
        $('.cart-con-info').hide();
    })
    //全部选中商品删除
    $(".qxdelete").click(function(){
        for(var i=0 ; i<$("input[class='mid-ipt f-l']").length;i++){
            if($("input[class='mid-ipt f-l']").eq(i).attr('checked')){
                $("input[class='mid-ipt f-l']").eq(i).parent().remove();
                i--;
            }
        }
        for(var k=0; k<$(".info-top").length;k++){
            if(!$(".info-top").eq(k).siblings().hasClass("info-mid")){
                $(".info-top").eq(k).remove();
                k--;
            }
        }
    })
    ...
    $('body').click(function(){
        var allPrice=0;
        for(var i in $('p[class="mid-je f-l"]')){
            var onePrice=$('p[class="mid-je f-l"]').eq(i).text().slice(1);
            $('input[class="mid-ipt f-l"]').eq(i).attr('checked')?allPrice+=parseInt(onePrice):"";
        }
        $('p[class="jshj-p2"]').children('span').html(allPrice);
    })
