...
class GoodsController extends Controller {
    public function goodsdetail(){
        ...
        $sql=isset($_SESSION['mname'])?"select goods_name,goods_pic from dr_goods g,dr_browse b where g.goods_id=b.goods_id and b.member_id='{$mid}'":"select id from dr_browse";
    ...
    public function colet(){
        ...
        $res=M('store')->where("goods_id={$data['goods_id']}")->select();
        if(session('mid')&&session('mname')){
            if(!$res){
                ...
                M('store')->where("goods_id={$data['goods_id']}")->delete();
                $result['status']=0;
                echo json_encode($result);
            }
        }else{
