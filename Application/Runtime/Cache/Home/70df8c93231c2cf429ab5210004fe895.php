    ...
    <div class="order-right f-r">
        <div class="order-hd">
            <dl class="f-l">
                ...
                <dd>
                    <h3><a href="#"><?php echo ($_SESSION['mname']); ?></a></h3>
                <!--    <p id="txt"><?php echo (date('y-m-d H:i:s,$res=time()')); ?></p>-->
                </dd>
                <div style="clear:both;"></div>
            </dl>
            <div class="ord-dai f-l">
                <form>
                <?php if(is_array($one)): foreach($one as $k=>$val): ?><p><a href="#" class="myorder_num" data-id="<?php echo ($val[order_status]); ?>">待付款<span><?php echo ($val['count(order_id)']); ?></span></a></p><?php endforeach; endif; ?>

                <?php if(is_array($two)): foreach($two as $k=>$val1): ?><p><a href="#" class="myorder_num" data-id="<?php echo ($val1[order_status]); ?>">待发货<span><?php echo ($val1['count(order_id)']); ?></span></a></p><?php endforeach; endif; ?>

                <?php if(is_array($three)): foreach($three as $k=>$val2): ?><p><a href="#" class="myorder_num" data-id="<?php echo ($val2[order_status]); ?>">待收货<span><?php echo ($val2['count(order_id)']); ?></span></a></p><?php endforeach; endif; ?>

                <?php if(is_array($foue)): foreach($foue as $k=>$val3): ?><p><a href="#" class="myorder_num" data-id="<?php echo ($val3[order_status]); ?>">待评价<span><?php echo ($val3['count(order_id)']); ?></span></a></p><?php endforeach; endif; ?>
                    </form>
            </div>
            <div style="clear:both;"></div>
        </div>
            ...
            <?php if(is_array($res)): foreach($res as $key=>$v): ?><div class="md-info">
                    ...
                    <div class="dai-con">
                        ...
                        <div class="dai-right f-l">
                            <P class="d-r-p1">颜色：<?php echo ($v[goods_color_name]); ?><br />尺码：<?php echo ($v[goods_size_name]); ?></P>
                            <p class="d-r-p2">¥ <?php echo ($v[goods_price]); ?></p>
                            <p class="d-r-p3">¥ <?php echo ($v[order_price]); ?></p>
                            <p class="d-r-p4"><a href="#">移入收藏夹</a><br />
                                <a href="<?php echo U('Order/pay');?>">付款</a></p>
                        </div>
                        <div style="clear:both;"></div>
                    </div>
...
<script>
    $(function(){
        ...
        $('.myorder_num').on('click',function(){
            ...
            if(tid>0){
                $.post("<?php echo U('Member/myorder_1');?>",{"tid":tid},function(data){
                    $("#myorder_1").html(data);
                })
            }
        })
