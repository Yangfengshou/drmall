<?php
//admin 分类查询
function getCateList($pid){
    $catelist=M('cate')->where("cate_pid=$pid")->select();
    return $catelist;
}
//根据id查数据
function getCateName($id){
    $catename=M('cate')->where("cate_id=$id")->select();
    return $catename;
}
function getBrandList($pid){
    $brandlist=M('brand')->where("brand_pid=$pid")->select();
    return $brandlist;
}
function getgroupList($type){
    $getgroupList=M('auth_group')->where("type=$type")->select();
    return $getgroupList;
}
 ...
 * 某用户是否有此权限
 */
function group_id2rule($group_id,$rule_id){
